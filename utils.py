...
def mc_sim(investment,num_periods,num_sims,mu,sigma,dt=1):
    St=pd.DataFrame(0.,index=list(range(1,num_periods+1)),columns=list(range(1,num_sims+1)))
    St.iloc[0]= investment
    for i in range(1,num_periods):
        dS_2_S=mu*dt+sigma*np.sqrt(dt)*np.random.randn(num_sims)
